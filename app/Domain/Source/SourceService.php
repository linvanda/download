...
class SourceService
{
     ...
     * 获取数据
     */
    public function fetch(Source $source, Target $target)
    {
        // 获取动态元数据
        $this->fetchDynamicMeta($source, $target);
        // 获取数据
        $source->fetch(new API(), new LocalFile($source->fileName()));
    }
     ...
     */
    private function fetchDynamicMeta(Source $source, Target $target)
    {
