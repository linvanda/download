...
1. 投递任务（异步下载）：POST /v1/task。需要 token 鉴权。
...
- **单表格**：一页 excel 只有一个表格；
- **多表格**：一页 excel 有多个表格。**多表格模式**仅支持通过 source_data 传输源数据（参见相关接口）；
...
// 即 PHP 中的二维数组，其它语言中的 Map 数组
// 其中的 key(name,age,...)对应 template 中列表头叶子节点 name 的值。
```

**同时有行表头和列表头时的数据格式：**

可以有两种格式（对应 PHP 中的二维数组和三维数组）：
...
// 三维数组格式，用 template 中行表头（row）叶子节点的 name 值作为第一维的 key
```
...
### 服务器健康监控：
服务器会启动 Defender 进程执行一些健康监控和数据清理，包括队列健康监控、失败任务重试、数据库历史数据归档、无用目录和文件清理，其中前三项仅在主服务器上执行。

可以在配置中心配置哪一台是主服务器（master_server），主服务器启动时会在 storage/temp/ 下生成 master_defender.txt 文件，记录了最近一次主服启动时间，主服正常关闭时会删除该文件。